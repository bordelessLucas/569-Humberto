...
export function StoragePage() {
  ...
  return (
    <div>
      <PageHeader title="Armazenamento" lede="Espaço usado, retenção e arquivos protegidos. A exclusão automática não toca no que foi protegido." />
      <div className="mt-6 flex flex-col gap-4">
        <QueryState isLoading={storage.isLoading} error={storage.error}>
          {overview ? (
            <section className="rounded-2xl border border-line bg-white p-5">
              ...
              <p className="text-sm leading-6 text-ink-muted">Retenção de {overview.policy.retentionDays} dias. Exclusão automática {overview.policy.autoDelete ? 'ligada' : 'desligada'}.</p>
              {canEdit ? <button className="btn btn-danger" type="button" onClick={() => retention.mutate()}>Simular exclusão</button> : <p className="text-sm leading-6 text-ink-muted">Somente leitura.</p>}
